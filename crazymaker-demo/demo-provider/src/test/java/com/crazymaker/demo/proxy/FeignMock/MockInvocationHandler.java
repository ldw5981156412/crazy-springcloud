...
public class MockInvocationHandler implements InvocationHandler {
    ...
    public static <T> T newInstance(Class<T> clazz) {
        ...
        for (Method method : clazz.getMethods()) {
            /*
             *  获取方法上的注解，判断是否为 Feign 的 @GetMapping 注解
             */
            Annotation getMappingAnno = method.getAnnotation(GetMapping.class);
             ...
             */
            String url = ((GetMapping) getMappingAnno).name();
            /*
             * 组装 MockRpcMethodHandler 模拟方法处理器
             * 注入 REST 地址的 contextPath 部分和uri部分
             */
            MockRpcMethodHandler rpcMethodHandler = new MockRpcMethodHandler(contextPath, url);
            /*
             *将模拟方法处理器handler 实例缓存到 dispatch 映射中
             */
            invocationHandler.dispatch.put(method, rpcMethodHandler);
        }
